...
void G_ShrinkPlayer (gentity_t *ent, gentity_t *attacker) {
	...
	// can't shrink team mates but can shrink self.
	if (attacker && OnSameTeam (ent, attacker )){
		if (ent != attacker && !g_friendlyFire.integer ) {
			return;
		}
	}
	if (ent->client->ps.powerups[PW_SHRINK]){
		if ( ent->client->ps.powerups[PW_SHRINK] - level.time - (SHRINK_TIME + SHRUNK_TIME) <= SHRINK_TIME 
		&& ent->client->ps.powerups[PW_SHRINK] - level.time - (SHRINK_TIME + SHRUNK_TIME) > 0){
			// still shrinking so don't do anything
			return;
		} else if (ent->client->ps.powerups[PW_SHRINK] - level.time <= SHRINK_TIME){
			// growing back up.  Shrink us back down from where we are.

			int timeRemaining;
			timeRemaining = ent->client->ps.powerups[PW_SHRINK] - level.time;
			ent->client->ps.powerups[PW_SHRINK] = level.time + (2 * SHRINK_TIME + SHRUNK_TIME) - timeRemaining;
			G_AddEvent( ent, EV_POWERUP_SHRINK, 0 );

		} else {
			// aready shrunk, so keep us tiny
			ent->client->ps.powerups[PW_SHRINK] = level.time + (SHRINK_TIME + SHRUNK_TIME);
		}
	} else {
		// Not shrunk yet so shrink us
		G_AddEvent( ent, EV_POWERUP_SHRINK, 0 );
		ent->client->ps.powerups[PW_SHRINK] = level.time + (2 * SHRINK_TIME + SHRUNK_TIME);
	}
}
